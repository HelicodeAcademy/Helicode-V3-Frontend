...
import {
  ...
  getCompanySupportedBanks,
  getCompanySupportedCountries,
  submitCompanyOfframpBank,
...
interface CompanyOfframpBankFormProps {
...
export function CompanyOfframpBankForm({
  ...
  const fetchCountries = async () => {
    try {
      ...
      const response = await getCompanySupportedCountries();
  ...
  const fetchBanks = async (countryCode: string, currencyCode: string) => {
    try {
      ...
      if (countryCode === "NG" || countryCode === "GH") {
        const response = await getCompanyQuidaxBanks(countryCode, "bank");
  ...
  const onSubmit = async (data: BankDetailsSubmissionData) => {
    try {
      ...
      const response = await submitCompanyOfframpBank(submitData);
