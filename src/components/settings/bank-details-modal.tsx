...
export function BankDetailsModal({ open, onOpenChange }: BankDetailsModalProps) {
    const [profile, setProfile] = useState<OfframpProfile | null>(null);
    ...
    useEffect(() => {
        ...
        const fetchProfile = async () => {
            ...
            setError(null);
            try {
                const data = await getOfframpProfile();
    ...
    const countryName = bank
        ? (new Intl.DisplayNames(["en"], { type: "region" }).of(bank.country) ?? bank.country)
        : "";
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-md gap-0 p-0 overflow-hidden" showCloseButton={false}>
                ...
                <div className="flex items-start justify-between px-6 pt-6 pb-4 border-b border-[#F2F2F2]">
                    <div>
                        <h2 className="text-lg font-semibold text-[#101928]">Bank payout details</h2>
                        <p className="text-sm text-[#667085] mt-0.5">Your linked bank account for payouts</p>
                    </div>
                    <button
