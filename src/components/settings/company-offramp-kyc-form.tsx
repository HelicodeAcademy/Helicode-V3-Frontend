...
export function CompanyOfframpKycForm({
  onSuccess,
  readOnly = false,
}: {
  onSuccess?: () => void;
  ...
  useEffect(() => {
    ...
    const fetchEnums = async () => {
      try {
        const data = await getCompanyOffRampEnums();
        setEnums(data);
      } catch (error) {
        const errorMessage =
  ...
  const onSubmit = async (data: KYCSubmissionData) => {
    try {
      ...
      await submitCompanyOfframpKyc(submitData);

      toast.success("Payout KYC submitted successfully!");
      onSuccess?.();
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : "Failed to submit KYC";
      toast.error(errorMessage);
      console.error("Company offramp KYC submission error:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
      ...
      <Button type="submit" disabled={isSubmitting || isLoading || readOnly} className="">
        {isSubmitting || isLoading ? "Submitting..." : "Save payout KYC"}
      </Button>
    </form>
  );
}
