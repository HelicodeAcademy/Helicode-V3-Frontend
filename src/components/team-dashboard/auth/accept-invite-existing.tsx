...
import { acceptTeamInviteExisting } from "@/lib/team/team-auth-service";
...
interface AcceptInviteExistingFormData {
  otp: string;
  email: string;
}
...
export function AcceptInviteExistingForm() {
  ...
  } = useForm<AcceptInviteExistingFormData>({
    defaultValues: {
      otp: "",
      email: "",
    },
  ...
  const onSubmit = async (data: AcceptInviteExistingFormData) => {
    try {
      setIsLoading(true);

      await acceptTeamInviteExisting({
        otp: data.otp,
        email: data.email,
      });
