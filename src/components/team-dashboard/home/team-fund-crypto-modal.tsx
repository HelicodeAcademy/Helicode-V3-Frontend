...
import { getTeamWalletAddress } from "@/lib/team/team-transaction-service";
import toast from "react-hot-toast";
import { Loader2, AlertCircle } from "lucide-react";
import { WalletData } from "@/store/wallet-store";
import { TeamMeResponse } from "@/store/team/team-auth-store";

interface TeamFundCryptoModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  teamData?: TeamMeResponse | null;
}

export function TeamFundCryptoModal({
  open,
  onOpenChange,
  teamData,
}: TeamFundCryptoModalProps) {
  ...
  const fetchWalletData = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const data = await getTeamWalletAddress();
      setWalletData(data);
    } catch (error) {
      const errorMessage =
        ...
        {isLoading ? (
          ...
          <div>
            ...
            <Link href="/team/dashboard">
              <Button className="mt-10">Back to Dashboard</Button>
            </Link>
          </div>
        ) : (
          <>
            <div className="mt-6">
              <div className="text-[#0052FF] text-xs font-medium bg-[#EFF4FF] p-2 rounded-sm flex items-center gap-2">
                ...
                <span>
                  ...
                  {network}. Only send {token} on {network} to the Wallet
                  Address
                </span>
              </div>
