    ...
    <div className="relative w-full min-h-50">
      {order.map((card, index) => {
        ...
        return (
          <motion.div
            key={card.id}
            className={`absolute w-full rounded-xl border border-[#E6E8EC] shadow-[0_4px_10px_-5px_rgba(0,0,0,0.10)] p-4 ${
              card.bg
            } text-black`}
            animate={{
              y: isFront ? 0 : 60,
              ...
              {card.rows.map((row) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    />
                    <span className="text-xs lg:text-sm font-medium text-[#A3A8B2]">
                      {row.currency}
                    </span>
                  </div>
